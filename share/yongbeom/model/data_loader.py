...
class FaceMaskDataset(Dataset):
    ...
    def __init__(self, csv_file, transform=None, target=None):
        ...
        _df["person"] = _df["person"].astype(int)

        label_normalized_dict = _df["label"].value_counts(
            normalize=True).to_dict()
        self.label_weight = [
            round(1 / (label_normalized_dict[label_idx] * 227.71), 4)
    ...
    def __getitem__(self, idx):
        ...
        if self.target:
            if self.target == "gender":
                ...
                _label = torch.tensor([_person, _mask, _label],
                                      dtype=torch.long)
        else:
            _age = self.age[idx]
            ...
            _mask = self.mask[idx]
            _label = self.label_list[idx]
            _label = torch.tensor([_age, _gender, _mask, _label],
                                  dtype=torch.long)
