...
class Classification_parallel(nn.Module):
    def __init__(self, model_name, device):
        ...
        class_num_final = [3125, 2489, 574, 4618, 4365, 580, 1336, 1369, 473, 2765, 1440, 251, 941, 885, 274, 1742, 1125, 176]
        class_num_mask = [15751, 7634, 5143]
        class_num_gender = [11466, 17062]
        class_num_age = [14527, 11673, 2328]
        for i in range(len(class_num_final)):
            class_num_final[i] = 28528/(18 * class_num_final[i])
        for i in range(len(class_num_mask)):
            class_num_mask[i] = 28528/(3 * class_num_mask[i])
        for i in range(len(class_num_gender)):
            class_num_gender[i] = 28528/(2 * class_num_gender[i])
        for i in range(len(class_num_age)):
            class_num_age[i] = 28528/(3 * class_num_age[i])
        ...
        feature_extractor_mask = [m for n, m in self.model_mask.named_parameters() if "classifier" not in n]
        classifier_mask = [p for p in self.model_mask.classifier.parameters()]
        params_mask = [
                {"params": feature_extractor_mask, "lr": self.lr * 0.1},
                {"params": classifier_mask, "lr": self.lr}
                ]
        feature_extractor_age = [m for n, m in self.model_age.named_parameters() if "classifier" not in n]
        classifier_age = [p for p in self.model_age.classifier.parameters()]
        params_age = [
                {"params": feature_extractor_age, "lr": self.lr * 0.1},
                {"params": classifier_age, "lr": self.lr}
                ]
        feature_extractor_gender = [m for n, m in self.model_gender.named_parameters() if "classifier" not in n]
        classifier_gender = [p for p in self.model_gender.classifier.parameters()]
        params_gender = [
                {"params": feature_extractor_gender, "lr": self.lr * 0.1},
                {"params": classifier_gender, "lr": self.lr}
                ]
        self.optimizer_mask = optim.AdamW(params_mask, lr= self.lr)
        self.optimizer_age = optim.AdamW(params_gender, lr= self.lr)
        self.optimizer_gender = optim.AdamW(params_age, lr= self.lr)
